...
ImageKnob::~ImageKnob()
{
    if (stitchedImage)
    {
        if (ImageCache::isImageInCache (stitchedImage))
            ImageCache::release (stitchedImage);
    }
}
...
{
    jassert (stitchedImage != 0); // you must first assign a stitchedImage to be
